...
static size_t rmt_encode_musical_score(rmt_encoder_t *encoder,
                                       rmt_channel_handle_t channel,
                                       const void *primary_data,
                                       size_t data_size,
                                       rmt_encode_state_t *ret_state) {
  rmt_musical_score_encoder_t *score_encoder =
      __containerof(encoder, rmt_musical_score_encoder_t, base);
  ...
  uint32_t rmt_raw_symbol_period = score_encoder->resolution / score->freq_hz;
  uint32_t rmt_raw_symbol_on = 10;
  uint32_t rmt_raw_symbol_off = rmt_raw_symbol_period - rmt_raw_symbol_on;
  ...
  size_t encoded_symbols =
      copy_encoder->encode(copy_encoder, channel, &musical_score_rmt_symbol,
...
esp_err_t
rmt_new_musical_score_encoder(const musical_score_encoder_config_t *config,
  ...
  ESP_GOTO_ON_FALSE(score_encoder, ESP_ERR_NO_MEM, err, TAG,
                    "no mem for musical score encoder");
  ...
  ESP_GOTO_ON_ERROR(
      rmt_new_copy_encoder(&copy_encoder_config, &score_encoder->copy_encoder),
