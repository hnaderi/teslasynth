...
class TestEnvelope:
    def test_adsr_fields(self):
        from teslasynth import get_instrument, InstrumentId
...
class TestConfiguration:
    ...
    def test_channel_access(self):
        from teslasynth import Configuration, ChannelConfig
        cfg = Configuration()
        for i in range(8):
            ch = cfg.channel(i)
...
class TestGetInstrument:
    def test_returns_instrument_info(self):
        from teslasynth import get_instrument, InstrumentId, InstrumentInfo
    ...
    def test_get_all_instruments_types(self):
        from teslasynth import get_all_instruments, InstrumentInfo
...
class TestGetPercussion:
    def test_returns_percussion_info(self):
        from teslasynth import get_percussion, PercussionId, PercussionInfo
        info = get_percussion(PercussionId.Kick)
        assert isinstance(info, PercussionInfo)

    def test_fields_populated(self):
        from teslasynth import get_percussion, PercussionId
    ...
    def test_get_all_percussions_types(self):
        from teslasynth import get_all_percussions, PercussionInfo
        for info in get_all_percussions():
            assert isinstance(info, PercussionInfo)

    def test_percussion_for_note(self):
        from teslasynth import percussion_for_note, PercussionInfo
...
class TestBuildInfo:
    def test_returns_build_info(self):
        from teslasynth import build_info, BuildInfo
        info = build_info()
...
class TestTeslasynth:
    ...
    def test_configured_construction(self):
        from teslasynth import Teslasynth, Configuration
        cfg = Configuration()
        cfg.set("synth.tuning=432hz")
        s = Teslasynth(cfg)
        assert s is not None
    ...
    def test_sample_all_pulses_are_pulse_objects(self, simple_midi):
        from teslasynth import Teslasynth, Pulse
    ...
    def test_handle_note_on(self):
        from teslasynth import Teslasynth, MidiChannelMessage
        s = Teslasynth()
