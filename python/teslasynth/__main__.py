...
def _find_instrument_or_percussion(name_or_id: str):
    ...
    try:
        ...
        _die(
            f"unknown instrument or percussion '{name_or_id}'.\n"
            f"Instruments:\n  {inst_names}\nPercussions:\n  {perc_names}"
        )
...
def _cmd_plot(args: argparse.Namespace) -> None:
    ...
    from teslasynth import plot, render
...
def _cmd_instruments(args: argparse.Namespace) -> None:
    ...
    for info in all_info:
        ...
        if env.type == "const":
            ...
            detail = (
                f"A={env.attack_ms:.0f}ms  D={env.decay_ms:.0f}ms"
                f"  S={env.sustain:.2f}  R={env.release_ms:.0f}ms"
                f"  curve={env.curve}"
            )
        else:  # ad
            detail = (
                f"A={env.attack_ms:.0f}ms  D={env.decay_ms:.0f}ms  curve={env.curve}"
            )
...
def _cmd_percussions(args: argparse.Namespace) -> None:
    ...
    for info in all_info:
        ...
        print(
            f"  {info.index:2d}  {info.name:{name_w}}"
...
def main() -> None:
    ...
    # ── plot ──────────────────────────────────────────────────────────────────
    p = sub.add_parser(
        "plot", help="Full overview: piano roll + signal + frequency + duty"
    )
    p.add_argument("midi", help="Input .mid file")
    _add_config_arg(p)
    p.add_argument(
        "--out", metavar="FILE.html", help="Save to HTML instead of opening the browser"
    )
