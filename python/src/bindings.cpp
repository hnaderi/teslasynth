...
NB_MODULE(_teslasynth, m) {
  ...
  m.def(
      "version", []() -> std::string { return TESLASYNTH_VERSION; },
  ...
  nb::class_<PyEnvelope>(m, "Envelope")
      ...
      .def("__repr__", [](const PyEnvelope &e) {
        ...
        return "Envelope(type='" + e.type + "', curve='" + e.curve +
  ...
  m.def(
      "get_percussion", [](PercussionId id) -> nb::dict { return percussion_to_dict(id); }, "id"_a,
      "Return a dict describing one percussion preset.");
  ...
  m.def(
      "percussion_for_note",
      [](uint8_t note) -> nb::dict { return percussion_to_dict(midi_to_percussion(note)); },
      "note"_a, "Return the percussion preset dict for a given MIDI drum note (0–127).");
  ...
  nb::class_<MidiChannelMessage>(m, "MidiChannelMessage")
      ...
      .def_prop_rw(
          "channel", [](const MidiChannelMessage &m) { return static_cast<uint8_t>(m.channel); },
          [](MidiChannelMessage &m, uint8_t v) { m.channel = v; })
      .def_prop_rw(
          "data0", [](const MidiChannelMessage &m) { return static_cast<uint8_t>(m.data0); },
          [](MidiChannelMessage &m, uint8_t v) { m.data0 = v; })
      .def_prop_rw(
          "data1", [](const MidiChannelMessage &m) { return static_cast<uint8_t>(m.data1); },
      ...
      .def_static(
          "program_change",
          [](uint8_t ch, uint8_t value) { return MidiChannelMessage::program_change(ch, value); })
      .def_static(
          "pitchbend",
          [](uint8_t ch, uint16_t value) { return MidiChannelMessage::pitchbend(ch, value); })
      .def_static("control_change",
                  [](uint8_t ch, uint8_t cc, uint8_t value) {
                    return MidiChannelMessage::control_change(ch, static_cast<ControlChange>(cc),
                                                              value);
                  })
      .def("__repr__", [](const MidiChannelMessage &m) { return std::string(m); });
  ...
  nb::class_<Routing>(m, "RoutingConfig")
          ...
          [](Routing &r, nb::list lst) {
            ...
            for (auto &slot : r.mapping) {
              ...
              if (item.is_none()) {
                ...
                if (v < 0 || v >= 8)
                  throw nb::value_error("output index must be in [0, 7]");
      ...
      .def("__repr__", [](const Routing &r) {
        ...
        s += "]";
        return "RoutingConfig(percussion=" + std::string(r.percussion ? "True" : "False") +
               ", mapping=" + s + ")";
      });
  ...
  nb::class_<SynthConfig>(m, "SynthConfig")
      .def(nb::init<>())
      .def_prop_rw(
          "tuning_hz", [](const SynthConfig &s) { return static_cast<float>(s.tuning); },
          [](SynthConfig &s, float v) { s.tuning = Hertz(v); }, "Concert tuning (default 440 Hz)")
      .def_prop_rw(
          "instrument", [](const SynthConfig &s) -> std::optional<uint8_t> { return s.instrument; },
  ...
  nb::class_<Config>(m, "Configuration")
      .def(nb::init<>())
      .def_prop_rw(
          "synth", [](Config &c) -> SynthConfig & { return c.synth(); },
          [](Config &c, const SynthConfig &s) { c.synth() = s; }, nb::rv_policy::reference_internal)
      .def_prop_ro("channels_size", &Config::channels_size, "Number of output channels (8).")
      .def(
          "channel",
      ...
      .def_prop_rw(
          "routing", [](Config &c) -> Routing & { return c.routing(); },
          [](Config &c, const Routing &r) { c.routing() = r; }, nb::rv_policy::reference_internal,
  ...
  nb::class_<Pulse>(m, "Pulse")
      .def(
          "__init__",
          [](Pulse *self, uint16_t on_us, uint16_t off_us) {
      ...
      .def_prop_ro(
          "on_us", [](const Pulse &p) -> uint16_t { return p.on.micros(); },
          "Pulse-on duration in microseconds.")
      .def_prop_ro(
          "off_us", [](const Pulse &p) -> uint16_t { return p.off.micros(); },
      ...
      .def("__eq__", [](const Pulse &a, const Pulse &b) {
        return a.on.micros() == b.on.micros() && a.off.micros() == b.off.micros();
      });
  ...
  nb::class_<Synth>(m, "Teslasynth")
      ...
      .def(nb::init<const Config &>(), "cfg"_a, "Create a synth with the given configuration.")
      .def(
          "handle",
          ...
          "msg"_a, "time_us"_a, "Feed a MIDI message at the given absolute time (microseconds).")
      .def(
          "sample_all",
          [](Synth &s, uint32_t budget_us) -> nb::list {
            if (budget_us > 65535)
              throw nb::value_error("budget_us must be ≤ 65535 (Duration16 limit ~65 ms). "
            ...
            for (uint8_t ch = 0; ch < 8; ch++) {
              ...
              if (n >= Buffer::output_bufsize)
                PyErr_WarnEx(PyExc_RuntimeWarning,
                             ...
                             "Reduce step_us or the synthesis frequency.",
                             1);
              nb::list ch_pulses;
      ...
      .def("off", &Synth::off, "Silence all voices immediately.")
      .def("reload_config", &Synth::reload_config,
           "Apply configuration changes (also calls off()).")
      .def_prop_rw(
          "configuration", [](Synth &s) -> Config & { return s.configuration(); },
